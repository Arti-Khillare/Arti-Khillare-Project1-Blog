  ...
  try {
    ...
    if (!isValidRequestBody(requestBody)) {
      res
        ...
        .send({
          status: false,
          message: "Invalid request parameteres . please provide Blog details",
        });
      return;
    }
    ...
    if (!isValidobjectId(authorId)) {
      res
        ...
        .send({
          status: false,
          message: `${authorId} is Not a Valid authorId`,
        });
      return;
    }
  ...
  try {
    ...
    if (!isValidRequestBody(requestBody)) {
      res
        ...
        .send({
          status: false,
          message: "No parameters Passed. Blog unmodified",
          Data: blog,
        });
      return;
    }
    ...
    if (subcategory) {
      ...
      if (Array.isArray(subcategory)) {
        updateBlogData["$addToSet"]["subcategory"] = {
          $each: [...subcategory],
        };
      }
...
const deleteById = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .send({ status: false, message: "Blod Deleted Successfully" });
  } catch (error) {
