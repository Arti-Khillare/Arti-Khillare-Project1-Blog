  ...
  try {
    ...
    if (!Author)
      return res
        ...
        .send({
          status: false,
          msg: "Author Not Found , plz check Credintials",
        });
    ...
    let token = jwt.sign(
      {
        ...
        groupno: "15",
        exp: Math.floor(Date.now() / 1000) + 10 * 60 * 60, //Date.now() method returns the number of milliseconds elapsed since January 1, 1970 00:00:00 UTC
      },
    ...
    res
      .status(200)
      .send({ status: true, message: "Author login SuccesFull", data: token });
  } catch (error) {
